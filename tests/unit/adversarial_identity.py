...
from packages.signal_core.enums import SignalStatus, TradeDirection
...
class TestIdentitySeparation:
    def test_same_logical_id_different_content_same_fingerprint_not_true(self) -> None:
        logical = uuid4()
        SignalIdentity(logical_signal_id=logical, provider_identity=provider_source())
    ...
    def test_revision_id_equals_logical_id_fails_structural_invariant(self) -> None:
        ...
        with pytest.raises(ValueError, match="independence"):
            SignalRevision(
                revision_id=same_uuid,
                logical_signal_id=same_uuid,
                revision_number=1,
                previous_revision_id=None,
                canonical_snapshot=(),
                fingerprint="ignored",
    ...
    def test_fingerprint_never_derived_from_identity(self) -> None:
        ...
        assert (
            str(rev.logical_signal_id) != fp
        )  # identity string representation should not match fingerprint
    ...
    def test_content_change_preserves_identity(self) -> None:
        ...
        rev1 = SignalRevision(
            ...
            revision_number=1,
            previous_revision_id=None,
            canonical_snapshot=(("status", SignalStatus.COMPLETE),),
            fingerprint="ignored",
            created_at_utc=datetime(2024, 1, 1, tzinfo=UTC),
        )
        rev2 = SignalRevision(
            ...
            canonical_snapshot=(("status", SignalStatus.PARTIAL),),
            fingerprint="ignored",
            created_at_utc=datetime(2024, 1, 1, tzinfo=UTC),
        )
