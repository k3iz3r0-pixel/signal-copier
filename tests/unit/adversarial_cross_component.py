...
from packages.signal_core.domain import Signal, SignalInstruction, SignalEvent, SignalRevision, SignalIdentity, canonical_fingerprint
...
class TestCrossComponentIdentityConsistency:
    def test_identity_unchanged_across_revision_and_event(self) -> None:
        logical = uuid4()
        id_ref = SignalIdentity(logical_signal_id=logical, provider_identity=ProviderSource(provider_name="p", signal_reference="r"))
        ...
        event = SignalEvent(
            event_id=uuid4(), signal_identity=id_ref,
        ...
        assert rev.logical_signal_id == event.signal_identity.logical_signal_id == instruction.signal_identity.logical_signal_id == logical

    def test_fingerprint_independent_of_revision_metadata_across_event_and_instruction(self) -> None:
        snapshot = (("status", SignalStatus.COMPLETE),)
        fp = canonical_fingerprint(snapshot)
        rev = SignalRevision(
            revision_id=uuid4(), logical_signal_id=uuid4(),
            revision_number=5, previous_revision_id=uuid4(),
            canonical_snapshot=snapshot, fingerprint="ignored",
            event_reference_id=uuid4(), snapshot_version=7,
            created_at_utc=datetime(2024, 1, 1, tzinfo=UTC),
        )
    ...
    def test_revision_snapshot_with_mutable_dict_fails_at_revision_level(self) -> None:
        with pytest.raises(TypeError, match="unsupported"):
            SignalRevision(
                revision_id=uuid4(), logical_signal_id=uuid4(),
                revision_number=1, previous_revision_id=None,
                canonical_snapshot=(("bad", {"nested": True}),), fingerprint="ignored",
                created_at_utc=datetime(2024, 1, 1, tzinfo=UTC),
            )
    ...
    def test_all_three_components_reject_same_bad_value(self) -> None:
        ...
        with pytest.raises(TypeError, match="unsupported"):
            SignalRevision(
                revision_id=uuid4(), logical_signal_id=uuid4(),
                revision_number=1, previous_revision_id=None,
                canonical_snapshot=bad_snapshot, fingerprint="ignored",
                created_at_utc=datetime(2024, 1, 1, tzinfo=UTC),
            )
        # Event payload
        id_ref = identity()
